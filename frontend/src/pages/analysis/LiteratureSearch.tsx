          ...
          <div className="flex-1 min-w-0">
            ...
            {stats?.document_count === 0 && (
              <div className="p-4 rounded-metal mb-6"
                ...
                <p className="text-cyan-300 mb-2">문헌 데이터베이스가 비어있습니다.</p>
                <button
            ...
            {activeTab === 'search' && (
              <div className="space-y-6 max-w-4xl">
                  ...
                  {showSearchInfo && (
                    <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                      <div className="rounded-metal p-3"
                        ...
                        <h4 className="font-medium text-orange-300 mb-2">🔤 Sparse (BM25) - {(SPARSE_WEIGHT * 100).toFixed(0)}%</h4>
                      ...
                      <div className="rounded-metal p-3"
                        ...
                        <h4 className="font-medium text-cyan-300 mb-2">📊 Dense (BioBERT) - {(DENSE_WEIGHT * 100).toFixed(0)}%</h4>
                        <p className="text-gray-300 text-xs">
                ...
                <form onSubmit={handleSearch} className="metal-card p-6">
                  <h2 className="text-lg font-semibold text-gray-800 mb-4">의학 문헌 검색</h2>
                      ...
                      <div className="space-y-4">
                        {searchResults.results.map((result, index) => (
                          ...
                          >
                                ...
                                <div className="flex gap-4 text-xs">
                                  ...
                                  <div className="flex-1">
                                    <div className="flex justify-between mb-1">
                                      <span className="text-orange-600 font-medium">Sparse (BM25)</span>
                                      <span className="text-orange-600">{(result.hybrid_score.sparse_score * 100).toFixed(1)}%</span>
                                    </div>
                                  ...
                                  <div className="flex-1">
                                    <div className="flex justify-between mb-1">
                                      <span className="text-cyan-700 font-medium">Dense (BioBERT)</span>
                                      <span className="text-cyan-700">{(result.hybrid_score.dense_score * 100).toFixed(1)}%</span>
                                    </div>
                                    <div className="h-2 rounded-full overflow-hidden" style={{ background: 'rgba(0,0,0,0.3)' }}>
                                      <div
                            ...
                            <div className="text-sm text-gray-700 mb-3 leading-relaxed whitespace-pre-wrap p-3 rounded-metal" style={{ background: 'rgba(0,0,0,0.2)' }}>
                              {result.content}
                            </div>
            ...
            {activeTab === 'qa' && (
              <div className="space-y-6">
                <form onSubmit={handleQuestion} className="metal-card p-6 max-w-4xl">
                  <h2 className="text-lg font-semibold text-gray-800 mb-4">의학 질문하기</h2>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="metal-card p-6">
                        <div className="flex justify-between items-center mb-4">
                          <h2 className="text-lg font-semibold text-gray-800">답변</h2>
                          <div className="flex items-center gap-2">
                        ...
                        <div className="prose prose-sm max-w-none">
                          <pre className="whitespace-pre-wrap font-sans text-gray-700 p-4 rounded-metal text-sm max-h-96 overflow-y-auto" style={{ background: 'rgba(0,0,0,0.2)' }}>
                            {ragResult.answer}
                          </pre>
                        </div>
                      </div>
                      ...
                      {ragResult.sources.length > 0 && (
                        <div className="metal-card p-6">
                          <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-gray-800">참고 문헌</h2>
                          ...
                          <div className="space-y-3 max-h-64 overflow-y-auto">
                            {ragResult.sources.map((source, index) => (
                              ...
                              >
                                <div className="flex justify-between items-start mb-2">
                                  <div>
                                    <p className="font-medium text-sm text-gray-800">[{index + 1}] {source.title}</p>
                ...
                {!ragResult && !ragMutation.isPending && (
                  <div className="metal-card p-8 text-center text-gray-600 max-w-4xl">
                    의학 관련 질문을 입력하세요
                  </div>
                )}
              </div>
            ...
            {activeTab === 'academic' && (
              <div className="space-y-6">
                ...
                <form onSubmit={handleAcademicSearch} className="metal-card p-6">
                  <h2 className="text-lg font-semibold text-gray-800 mb-4">학술 논문 검색</h2>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-sm font-medium text-gray-700">검색 소스</label>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                      {sourcesData?.sources?.map((source: AcademicSource) => (
                        ...
                        >
                          ...
                          />
                          <div>
                            <p className="text-sm font-medium" style={{ color: selectedSources.includes(source.id) ? '#4ADE80' : '#1E293B' }}>{source.name}</p>
                            <p className="text-xs" style={{ color: selectedSources.includes(source.id) ? '#E2E8F0' : '#64748B' }}>{source.description}</p>
                          </div>
                        </label>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-6 mb-4">
                    {/* Max Results */}
                    <div className="flex items-center gap-2">
                      <label className="text-sm text-gray-600">소스당 최대 결과:</label>
                      <select
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="rounded text-accent-green"
                      />
                      <span className="text-sm text-gray-600">VectorDB에 자동 저장</span>
                          ...
                          articles.length > 0 && (
                            <div key={source} className="rounded-metal overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
                              <div className="px-4 py-2 flex items-center justify-between"
                                ...
                                <span className="text-sm text-gray-600">{articles.length}건</span>
                              </div>
                              <div className="divide-y divide-gray-200">
                                {(articles as AcademicArticle[]).map((article, index) => (
                                  ...
                                  >
                                    ...
                                    <p className="text-sm text-gray-700 mb-2 line-clamp-3">{article.abstract}</p>
